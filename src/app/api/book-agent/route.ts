  ...
  try {
    ...
    if (!body.prompt?.trim()) {
      return NextResponse.json(
        { error: "Prompt is required — type something!" },
        { status: 400 },
      );
    }
    ...
    if (!apiKey || apiKey === "") {
      return NextResponse.json(
        {
          error:
            "🚨 API Key Missing! Go to Vercel Settings > Environment Variables and add GROQ_API_KEY (your real key from console.groq.com)",
        },
        { status: 500 },
      );
    }
    ...
    const reply =
      ...
      "Sorry, no response from AI right now. Try again!";
